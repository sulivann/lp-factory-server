...
app.get('/build', (req, res) => {
  ...
  reqArray.forEach(component => {
    files.push(path.resolve(__dirname, `/components/${component}.html`));
  });

  // Head and end of file
  files.unshift(path.resolve(__dirname, '/components/head.html'));
  files.push(path.resolve(__dirname, '/components/end.html'));
  ...
  fileHelper.readFiles(files).then(htmlArray => {
    ...
    fs.mkdir(path.resolve(__dirname, `/public/${uuid}`), err => {
      if (err) {
        throw err;
      }

      const pathToFile = path.resolve(__dirname, `/public/${uuid}/index.html`);
